...
public class ParquetBucketMetadata<K, V> extends BucketMetadata<K, V> {
  ...
  public Map<Class<?>, Coder<?>> coderOverrides() {
    ...
    return ImmutableMap.of(
        ByteBuffer.class, AvroBucketMetadata.ByteBufferCoder.of(),
        CharSequence.class, AvroBucketMetadata.CharSequenceCoder.of()
    );
  }
  ...
  private static String validateKeyField(String keyField, Class keyClass, Class recordClass) {
    switch (getRecordType(recordClass)) {
      case AVRO:
        return AvroBucketMetadata.validateKeyField(
            keyField, keyClass, new ReflectData(recordClass.getClassLoader()).getSchema(recordClass));
      case SCALA:
  ...
  private static String validateScalaKeyField(String keyField, Class keyClass, Class recordClass) {
    ...
    for (int i = 0; i < keyPath.length - 1; i++) {
      try {
        ...
        throw new IllegalStateException(String.format(
            "Key path %s does not exist in record class %s", keyPath[i], current));
      }
